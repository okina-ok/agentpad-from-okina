...
## 1.5 布局与屏幕（v0.7.16 更新：4×4 方格定稿）

**4×4 方格（16 键位）**：物理按键 13 颗 + 1 旋钮，屏幕只占 1U。

| 排 | 列1 | 列2 | 列3 | 列4 |
| --- | --- | --- | --- | --- |
| 1 | 强度旋钮 | 频道1 | 频道2 | 屏幕（1U） |
| 2 | 频道3 | 频道4 | 频道5 | 频道6 |
| 3 | 确定 | 返回 | 新建 | 分支 |
| 4 | 停止 | 语音输入（2U 大键） | 通话 |

- **6 个频道键**：状态灯，紫=thinking / 青=running / 琥珀=waiting /
  绿=done / 红=error / 灰=idle，灯效 solid/breathe/flash 由 PC 下发。
- **第 3/4 排为可自定义预设键**：默认 确定/返回/新建/分支/停止/通话，
  接受/拒绝/重试等可后续重配（宏后置）。
- **语音大键（2U）**：按住录音 → 松开本地转写并填入对话框 →
  按"确定"发送（正版 Codex 无需对话框时亦可直发）。
- **1 个 EC11 旋钮**：强度/推理深度调节（v1 先只上报，不做动作）。
- **屏幕（1U，型号待定，候选 1.28"/1.3" 方屏）**：常驻显示最活跃频道
  状态 + 电量；按频道键后切详情页显示会话 summary。
...
- **频道绑定（v0.7.2+）**：`channel_map.json` 决定频道 ↔ 对话映射；
  ...
  `anchor_channel_1`（默认关）= 是否把频道 1 当锚定槽；普通用户所有频道平等，
  想固定主频道用 `manual` 绑定即可。
...
- `cmd`：命令键标签（v1 固定 6 颗自定义 + 语音大键，宏后置）。
